...
document.getElementById("arrow").onclick = function() {document.getElementById("WorkExpTab").click();};
(function($) {
...
.on('click', 'a', function(event) {
    ...
    var $a = $(this),
        ...
        $modalDesc = $modal.find('.description'),
        $modalIframeContainer = $modal.find('.iframe-container'), // Nuevo contenedor para el iframe
        href = $a.attr('href')
        description = $a.data('description'),
        iframeCode = $a.data('iframe'); // Obtener el iframe
    ...
    $modalIframeContainer.html(iframeCode || ""); // Si no hay iframe, deja el contenedor vacío.
...
.on('click', '.modal', function(event) {
    ...
    var $modal = $(this),
        $modalImg = $modal.find('.inner img'),
        $modalDesc = $modal.find('.description'),
        $modalIframeContainer = $modal.find('.iframe-container'); // También limpiar el iframe
    ...
    setTimeout(function() {
        ...
        setTimeout(function() {
            // Clear src, descripción e iframe.
            $modalImg.attr('src', '');
            $modalDesc.text('');
            $modalIframeContainer.html(''); // Eliminar el iframe
...
.prepend('<div class="modal" tabIndex="-1"><div class="inner"><div class="description"></div><div class="iframe-container"></div><img src="" /></div></div>')
.find('img')
...
// Abre el popup y carga el video cuando se haga clic en un enlace con extensión .mp4 o .webm
videoLinks.forEach(function(videoLink) {
    videoLink.addEventListener('click', function(event) {
        const videoSrc = videoLink.getAttribute('href');
        
        // Verifica que el archivo sea .mp4 o .webm
        if (videoSrc.endsWith('.mp4') || videoSrc.endsWith('.webm')) {
            event.preventDefault(); // Prevenir el comportamiento por defecto del enlace
            popup.style.display = 'flex'; // Mostrar el popup
            popupVideo.src = videoSrc; // Establecer la fuente del video
            popupVideo.play(); // Reproducir el video automáticamente
        }
    });
});

// Cierra el popup al hacer clic en el fondo
popup.addEventListener('click', function(event) {
    if (event.target === popup) {
        popup.style.display = 'none'; // Ocultar el popup
        popupVideo.pause(); // Pausar el video
        popupVideo.currentTime = 0; // Reiniciar el video al inicio
    }
});
